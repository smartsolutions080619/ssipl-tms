...
export class AddUserStatus1700000000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      UPDATE tenant_ssipl.users SET status = 'ACTIVE' WHERE is_active = true AND deleted_at IS NULL
    `);
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`ALTER TABLE tenant_ssipl.users DROP COLUMN IF EXISTS status`);
  }
}